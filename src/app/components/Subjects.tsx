...
<label className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Aggingi Materie già create</label>
<div className="flex">
<select value={subjectId} onChange={(e)=>{setSubjectId(parseInt(e.target.value))}} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
  ...
  {subjectNoList.map((subject:Subject)=>(<option key={subject.id} value={subject.id}>{subject.name}</option>))}
</select>
<button
                            className="ml-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
                            onClick={()=>{addSubjectAlreadyExists()}}
                        >
                            {`Aggiungi`}
                        </button>
</div>
...
<div className="subjects">
  <ul className="list-disc">
    {subjectList.map((subject:Subject,i:number) =>(
      <li key={i} className="flex justify-between">
        {subject.name}
        <div className="flex">
        <svg onClick={()=>{modify(subject.id,subject.name)}} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 icon">
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18 18 6M6 6l12 12" />
                        ...
                        {/* Titolo */}
                        <div className="mb-4">
                            <h2 className="text-lg font-semibold">{subjectId === -1 ? `Aggiungi Materia`:`Modifica Materia`}</h2>
                        </div>
